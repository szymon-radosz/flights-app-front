...
const ListGrid = ({ isHomePage }: Props) => {
  const scrollAnimationHomePage = useMemo(() => getScrollAnimation(), []);
  ...
  return (
    // <section >
    <ScrollAnimationWrapper className='pt-4 pb-10 lg:pt-10'>
      <motion.div variants={isHomePage ? scrollAnimation : {}}>
        ...
        <div className='mx-auto grid max-w-6xl  grid-cols-1 gap-6 pt-3 pb-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4'>
          {list?.map((singleListItem, i) => {
            return (
              <SingleListGrid cityTo='Londyn' cityFrom='Warszawa' key={i} />
            );
          })}
        </div>
      </motion.div>
